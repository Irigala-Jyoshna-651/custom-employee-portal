...
// GET /api/auth/me - return user with roles and permissions
router.get('/me', async (req, res, next) => {
  try {
    ...
    for (const ur of user.roles) {
      const role = ur.role;
      if (role && role.permissions) for (const rp of role.permissions) if (rp.permission && rp.permission.name) perms.add(rp.permission.name);
    }

    res.json({ id: user.id, email: user.email, createdAt: user.createdAt, roles, permissions: Array.from(perms) });
  } catch (err) {
    next(err);
  }
});
